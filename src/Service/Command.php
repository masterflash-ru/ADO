...
public function Execute (&$RecordsAffected = 0, &$Parameters = NULL,   $Options = adCmdText)
	...
	if (! is_object($this->ActiveConnection)) { // имеем строку, генерируем объет
		...
		// выполняем запрос
		// ($CommandText,&$RecordsAffected=0,$Options=adCmdText,$parameters=NULL)
...
public function CreateParameter ($Name, $Type, $Direction, $Size, $Value,	$Attributes = adParamNullable)
	...
	$Attributes  -  атрибуты */
		//echo $Attributes & ( adParamSigned + adParamNullable + adParamLong); echo ' ';
		//проверим валидность
		if (!in_array($Type,$this->ParameterType)) throw new ADOException($this->ActiveConnection, 17, 'Command:', array('Type='.$Type));
		if (! ($Attributes & ( adParamSigned + adParamNullable + adParamLong))) throw new ADOException($this->ActiveConnection, 17, 'Command:', array('Attributes='.$Attributes));
