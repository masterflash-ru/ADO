...
class RecordSet implements Iterator
{
    ...
    public function Open ($Source = null, $ActiveConnect = null,  $Options = adCmdText)
    {
        ...
        if (is_null($ActiveConnect) && $this->ActiveConnect instanceof Connection)  {
            ...
            if ($this->container['source'] instanceof Command) {
                $this->container['source']->ActiveConnection = $this->ActiveConnect;
            }
        } else {
            if ($ActiveConnect instanceof Connection) {
                // если тип соединения верный, записать его в недра RecordSet
                if ($this->container['source'] instanceof Command) {
                    $this->container['source']->ActiveConnection = $ActiveConnect;
                }
    ...
    { 
        ...
        if (is_string($start)) {
            ...
            $this->jmp_record($this->container['absoluteposition'] + $NumRecords); // переходим теперь к указаной записи, получается по отношению к закладке
        } else { 
            ...
            switch ($start) {
                ...
                default: {// по умолчанию от текущей записи
                    $this->jmp_record($this->container['absoluteposition'] + $NumRecords);
                }
            }
        }
    }
    ...
    { // переход на абсолютную запись с номером AbsolutePosition, 1.....
        ...
        if (  // условие при котором мы попадем в кеш
                ...
                (count($this->rez_array) <=
                         ...
                         $this->container['maxrecords'] == 0
                ) && // границы  кеша,  если  он  ограничен  касаемо  объема  его  и  кол-ва  записей  в  нем  уже  имеющихся
            ...
            if ($this->container['absoluteposition'] > $this->RecordCount)  {
                $this->EOF = true;
            } else {
                $this->EOF = false;
            }
        ...
        if ($this->container['maxrecords'] > 0)  {
            $Recorditem = $this->container['maxrecords']; // указан  максимум  записей  в  кеше
        } else {
            $Recorditem = $this->RecordCount; // по умолчанию грузим все проверим допустимость верхний предел
        }
        ...
        if ($Recorditem > $NewAbsolutePosition &&  $NewAbsolutePosition < $this->container['absoluteposition']) {
            $Recorditem = $NewAbsolutePosition;
        }
        ...
        if ($NewAbsolutePosition > $this->container['absoluteposition'] || 
            (isset($this->rez_array[$NewAbsolutePosition -  $this->AbsolutePosition_min_max[0]]) && 
             $this->rez_array[$NewAbsolutePosition -  $this->AbsolutePosition_min_max[0]]['status']['flag_deleting']))  {
            ...
            if (! $this->EOF)  {
                $this->jmp_record($NewAbsolutePosition); // рекурсивно обратимся, т.к. кеш уже заполнен
            }
            return;
        }// if ($NewAbsolutePosition > $this->container['absoluteposition']) 
        ...
        if ($NewAbsolutePosition < $this->container['absoluteposition']) { 
            ...
            for ($i = 0; $i < $Recorditem; $i ++) {
                ...
                if (isset($_SESSION['ADORecordSet'][$this->RecordSetId][$i + $NewAbsolutePosition - 1])) {
                    ...
                    // грузим из базы, и сбрасываем все флаги
                    if (!empty($this->stmt)) {
                        $rez_array[$i] = $this->container['source']->ActiveConnection->driver->fetchNext( $this->stmt); // считать   запись
                    }
                    $rez_array[$i]['status'] = [
                        'flag_delete' => false, 
                        'flag_change' => false, 
 ...
 public function AddNew ($Fields = null, $Values = null)
    ...
    */
     if ($this->container['cursortype'] == adOpenForwardOnly) {
         throw new ADOException($this->ActiveConnect, 3,  'RecordSet:' . $this->RecordSetName);
     }
    ...
    // создаем флаги для данной записи
     ...
     if (!empty($Fields)) {
         ...
         if (! empty($this->stmt))  {
             $this->Update(); // сразу  записать,  если  конечно  есть канал  для записи, если его  нет, тогда  игнорируем
         }
     }//if (! empty($Fields)) 
...
{ 
    ...
    if ($AffectRecords == adAffectGroup) {
        ...
        if (empty( $this->container['filter']))  {
            return;
        }
    ...
    if ($AffectRecords == adAffectAll && $this->container['maxrecords'] >  0) { 
        ...
        throw new ADOException($this->ActiveConnect, 7, 'RecordSet:' .$this->RecordSetName, array('UpdateBatch()'));// ошибка,  т.к.  не  все  записи  загружены
    } 
    $this->MoveFirst();
    if (! $MultiInsert) { 
        ...
        while (! $this->EOF)  {
            ...
            try {
                ...
                if (is_null($ADOException)) {
                    ...
                    $ADOException->errors->add(['number' => $e->getCode(), 'description' => $e->getMessage(), 'source' => "RecordSet"]);// $source
                }
            }
            $this->set_status();
            $this->MoveNext();
        ...
        if (is_object( $ADOException))  {
            throw new ADOException($ADOException);
        }
        return;
    ...
    foreach ($this->rez_array as $key => $rez_array) { 
        ...
        if (isset( $_SESSION['ADORecordSet'][$this->RecordSetId]))  { 
            foreach ($_SESSION['ADORecordSet'][$this->RecordSetId] as $k => $f) {
                ...
                $new = array_combine($this->get_field_name_false, $new);
                $old = array_combine($this->get_field_name_false, $old);
                $sql_s[] = $this->container['source']->ActiveConnection->create_sql_update($this->stmt, $old, $new, $status); // сгенерировать sql для update
            }// foreach ($_SESSION['ADORecordSet'][$this->RecordSetId] as $k => $f) 
        }//if (isset( $_SESSION['ADORecordSet'][$this->RecordSetId])) 
    }// foreach ($this->rez_array as $key => $rez_array) 
    ...
    if (count($sql_s)) { 
        ...
        $RecordsAffected = 0;
        try {
            $this->container['source']->ActiveConnection->Execute($sql, $RecordsAffected, adExecuteNoRecords, null); // просто исполинть и все
        } catch (ADOException $e) { 
            // если  возникла  ошибка,  сохраним  ее  в  запись  RS  где  она  произошла  и  выполним  еще  раз  исключение  для  отлова  ее  дальше
            $this->rez_array[$this->container['absoluteposition'] - $this->AbsolutePosition_min_max[0]]['status']['errors'] = $e;
            throw new ADOException( $e);
...
{ // удаляет записи
     if ($this->container['cursortype'] == adOpenForwardOnly) {
         throw new ADOException( $this->ActiveConnect,  4, 'RecordSet:' . $this->RecordSetName);
     }
     switch ($AffectRecords) {
         ...
         case adAffectGroup:{
             ...
             if (empty( $this->container['filter']))  {
                 return; // удалять нечего,  критерий  не задан
             }
             $this->MoveFirst();
             // пробежим по всем записям и пометим их
             while (! $this->EOF) {
 ...
 */
 if (empty($Rows)) {
     $Rows = adGetRowsRest; // по умолчанию, если вдруг  будет  пусто
 }
 if (empty($Field)) {
     $Field = [];
 }
 ...
 if (count( $Field) > 0)  {
     $f = array_intersect(  $f,  $Field); // оставить только те поля,который  мы указали в $Field
 }
 if (count($f) == 0)  {
     return []; // нет  полей  для  вборки,  выходим
 }
    ...
    while (!$this->EOF) {
        ...
        foreach ($f as $number => $name) {
            if ($Rows & adGetRowsArrType) {
                ...
                $rez[$number ][] = $this->Fields->Item[$number]->Value;
            }
        }
        $this->MoveNext(); 
    }
...
{ // настраивает RS в соответсвии со схемой $source - либо строка с самой схемой либо имя файла
    ...
    if (strrpos($source, '<') !== false) {
        $io->source = $source;
    } else { // исходные данные в файле, проверим его наличие, если нет, ошибка
        if (is_file($source) && ! is_dir($source)) {
            $io->source = file_get_contents( $source);
        } else {
...
{ // аналогично GetXmlSchema(), но выводит в destination
    ...
    if (is_string($destination)){
        file_put_contents(    $destination, $xsd);
    } else {
        return $xsd;
    }
}
...
*/
    ...
    foreach ($s1 as $s) {
        ...
        if (! isset( $sort[$i][1])) {
            $sort[$i][1] = 'asc'; // если не задан пераметр, значит по умолчанию это asc
        }
        $i ++;
      }
...
{ // поиск по закладке
    ...
    while (! $this->EOF) {
        if ($this->container['bookmark'] ==  $BookMark){
            // нашли, в рекордсете текущая запись точо соотвествует закладке
            return; 
        } 
        $this->MoveNext();
    }
...
{ // получить  массив  имен  полей  $type-  тип  выхода  false:  array(0=>имя,....),  иначе  array(имя=>номер  по  порядку)
    if (!isset( $this->rez_array[0])) {
        return []; // если рекордсет пустой, то выход
    }
    $i = 0;
    $field_name = [];
    if ($type) {
...
{ // установка  кодов  статуса  исходя  из  внутреннего  массива  статусов
    ...
    if ($status['preserveptatus'])  {
        return;
    }
    if ($status['flag_new']) {
        $this->Status += adRecNew;$this->EditMode=adEditAdd;
    }
    if ($status['flag_change'])  {
        $this->Status += adRecModified;$this->EditMode=adEditInProgress;
    }
    if ($status['flag_canceled']) {
        $this->Status += adRecCanceled;$this->EditMode=adEditNone;
    }
    if ($status['flag_delete']) {
        $this->Status += adRecDeleted;$this->EditMode=adEditDelete;
    }
}
