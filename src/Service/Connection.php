...
{ // объект cоnnection - содинение к базе данных текущего провайдера
	...
	{ 
		...
		if (!empty($Provider)) $this->Provider=$Provider;
...
if ($server)   $dsna['host'] = $server; // print_r($dsna);
if ($user)   $dsna['user'] = $user;
if ($password)   $dsna['pass'] = $password;
if ($database)   $dsna['path'] = $database;
if (!empty($dsna['scheme']))   {$this->Provider = $dsna['scheme'];}
	else 
		{
			if ($this->Provider) $dsna['scheme']=$this->Provider ;
		}

if (! $this->driver) // инициализируем драйвер, т.к. конструктор был вызван без указани
	{
		$d='ADO\\Drivers\\'.$this->Provider;
		$this->driver = new $d;
	  }
...
public function Execute ($CommandText, &$RecordsAffected = 0, $Options = adCmdText, $parameters = NULL)
	...
	//echo "CommandText=$CommandText<br>";
	...
	$rez = $this->driver->Execute($this->connect_link, $CommandText, 	$Options, $parameters);
		...
		//echo $CommandText.";".$parameters."<br>";
		if (! empty($rez['error'])) 
			{ // print_r($rez['error']->errorInfo[1]);
				$this->Errors->add(	array('number' => $rez['error']->errorInfo[1],  // код ошибки драйвера
				...
				call_user_func_array($this->event['ExecuteComplete'], array($rez['RecordsAffected'], 		//затронуто рядов
		...
		if ($Options & adExecuteNoRecords) {return true;} else return $rez;
		// print_r($rez['stmt']->fetchAll());
		// $rs=new RecordSet();//создать новый RS
		// $rs->ActiveConnect=&$this;//сохранить объект соединени
...
public function functionRollbackTrans()
{
	...
	public function &__get ($var)
	{
		...
		if (isset($this->container[$var]))	return $this->container[$var];
		$arr = debug_backtrace();
			...
			case 'cursorlocation':
				{
					...
					if ($value > 0) {
						$this->container['cursorlocation'] = $value; // проверим на
																		  // допустимость
					}
					break;
				}
